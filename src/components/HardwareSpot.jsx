...
export default function HardwareSpot({ spot, object, setInspectedObject }) {
  ...
  const { spotId, standId, objectSN, hardwareType } = object || {};
  ...
  return (
    <div className={`flex flex-col items-center gap-2 absolute text-black text-center font-bold uppercase leading-4`} style={{ left: `${position.x * coordinateMultiplier}px`, top: `${position.y * coordinateMultiplier}px` }}>
      <div data-object-id={id} className={`cursor-pointer rounded-full w-[90px] h-[90px] object ${spot && hardwareId == '' ? 'bg-yellow-300' : `bg-green-500`}`} onClick={() => handleObjectClick(spot, object)}>
        <div className="flex flex-col h-full justify-center">
          ...
          <p>{object ? hardwareType != "Barrel" ? `${hardwareType} ${objectSN}` : '' : ''}</p>
